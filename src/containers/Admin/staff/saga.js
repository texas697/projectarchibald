...
const _delete = id => firebaseApp.database().ref().child(`${PATH_STAFF}`).orderByChild('id').equalTo(id).set(null)

const _post = model => {
  const newKey = firebaseApp.database().ref().child(PATH_STAFF).push().key
  const updates = {}
  updates[`/${PATH_STAFF}/${newKey}`] = model
...
function * _addRequest (action) {
  try {
    ...
    yield put(actions.fetchStaffRequest(action.model.teamId))
  } catch (error) {
