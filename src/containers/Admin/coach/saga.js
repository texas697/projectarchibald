...
const _delete = id => firebaseApp.database().ref().child(`${PATH_COACH}`).orderByChild('id').equalTo(id).set(null)

const _post = model => {
  const newKey = firebaseApp.database().ref().child(PATH_COACH).push().key
  const updates = {}
  updates[`/${PATH_COACH}/${newKey}`] = model
